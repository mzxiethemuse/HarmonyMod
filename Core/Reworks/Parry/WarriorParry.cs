...
public class WarriorParry : ModPlayer
{
    ...
    {
        parryCooldown = debug ? 10 : 60 * 2; //60 * 3;
    ...
    {
        if (ParryKey.parryKeybind.JustPressed)
        {
            if (parryCooldownTimer < 0)
            {
                OnParryKeyPressed();
    ...
    public void OnParryKeyPressed()
    {
        ...
        foreach (Projectile proj in Main.projectile.Where(proj => proj.active))
        {
            //TODO: replace this with something that takes hitboxes into accountq
